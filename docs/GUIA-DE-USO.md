...
El matraz que aparece en el encabezado, el pie de página y el favicon es
un único diseño (antes eran dos íconos distintos entre sí). Vive en dos
formatos:
- **Insignia** (`assets/icons/favicon.svg` y los PNG/ICO generados a
  partir de él): con fondo cuadrado redondeado en degradado, pensada para
  favicon e íconos de instalación (PWA).
- **Solo la marca**: sin fondo, con el contorno en `currentColor` (se
  adapta automáticamente al color del texto en cada tema) y el líquido
  con degradado violeta → teal — la que se usa inline en el encabezado y
  el pie de página.

Si alguna vez rehaces los íconos, mantén el mismo trazo (contorno recto,
esquinas redondeadas) para que ambos formatos sean reconocibles como el
mismo símbolo.
...
claro es el de siempre, sin cambios.

El oscuro usa **negro puro** como base con un fondo animado tipo espacio
detrás de las secciones, con varias capas independientes (nada de esto
afecta el rendimiento — son 5 elementos fijos, no escalan con el tamaño
del catálogo):
- Una nebulosa con 4 tonos (violeta, teal, violeta profundo y un toque de
  magenta) que "respira" muy lentamente (escala + desplazamiento sutil).
- Tres capas de estrellas de distinto tamaño, densidad y velocidad de
  desplazamiento — dan sensación de profundidad y titilan en momentos
  distintos entre sí, en vez de parpadear todas juntas.
- Una viñeta que oscurece los bordes para dar un aire más cinematográfico.
- Una estrella fugaz ocasional (cada ~16 segundos).

Las tarjetas y superficies se mantienen en tonos oscuros sólidos para que
el texto siga siendo legible sobre el fondo animado.

**Nota técnica — cortina de transición:** el "flash" (`#themeFlash`) que
cubre la pantalla durante el cambio de tema se limpia con un temporizador
simple (`setTimeout`), no con eventos como `transitionend` o
`Animation.onfinish` — se comprobó que esos eventos no siempre se disparan
a tiempo en todos los navegadores, lo que podía dejar la cortina cubriendo
la pantalla por varios segundos. El temporizador simple es menos elegante
pero garantiza que la cortina siempre desaparezca, sin excepción.
